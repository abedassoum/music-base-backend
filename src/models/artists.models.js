...
export async function readAllArtists_db() {
  const sql = `
    SELECT 
      ...
      GROUP_CONCAT(DISTINCT album.name) AS albums,
      GROUP_CONCAT(DISTINCT song.title) AS songs
    FROM artists
  ...
  try {
    ...
    const fullArtists = results.map(artist => ({
      ...artist,
...
export async function updateArtist_db(
  ...
  const sql = `
    ...
    SELECT id, ? FROM albums WHERE name IN (?);
...
export async function createArtist_db(
  ...
  const sql = ` 
    ...
    INSERT INTO album_artist (album_id, artist_id)
    SELECT id, @artistId FROM albums WHERE name IN (?);
